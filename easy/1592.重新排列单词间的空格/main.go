...
// author: fengyuwusong date: 2022-09-07 10:23:13
...
func reorderSpaces(text string) string {
	...
	splitNums := 0
	if wordNums != 1 {
		splitNums = spaceNums / (wordNums - 1)
	}
	lastNums := spaceNums - splitNums*(wordNums-1)
	splitSpaces := strings.Repeat(" ", splitNums)
	lastSpaces := strings.Repeat(" ", lastNums)
	return strings.Join(words, splitSpaces) + lastSpaces
}
